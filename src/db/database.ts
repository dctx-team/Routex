...
export class Database {
  ...
  private mapChannelRow(row: ChannelRow): Channel {
    ...
    assertValidChannelType(row.type);
    assertValidChannelStatus(row.status);

    return {
      id: row.id,
      name: row.name,
      type: row.type,  // ✅ Now type-safe: ChannelType
      ...
      weight: row.weight,
      status: row.status,  // ✅ Now type-safe: ChannelStatus
