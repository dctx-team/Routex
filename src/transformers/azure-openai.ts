...
export class AzureOpenAITransformer extends OpenAITransformer {
  ...
  async transformRequest(request: any, _options?: any): Promise<any> {
    ...
    // 使用 OpenAI 转换器作为基础
    const transformed = await super.transformRequest(request, _options);

    // Add Azure-specific parameters / 添加 Azure 特定参数
    if (_options?.dataSources) {
      transformed.data_sources = _options?.dataSources;
    }

    if (_options?.enhancements) {
      transformed.enhancements = _options?.enhancements;
    }

    // Azure content filtering / Azure 内容过滤
    if (_options?.contentFilter !== undefined) {
      transformed.content_filter = _options?.contentFilter;
    }
  ...
  async transformResponse(response: any, _options?: any): Promise<any> {
    ...
    const transformed = await super.transformResponse(response, _options);
